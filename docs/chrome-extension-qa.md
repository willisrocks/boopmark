...
- Before the actual toolbar click, the desktop locked and macOS denied assistive access. The native bridge failed closed with no click; the sanitized observer also made no request. No extension capture or bookmark write is claimed for this checkpoint. A read-only heartbeat now checks only the console lock state; it cannot control the browser or mutate production. Resume AC14 from the explicitly loaded build when the desktop is unlocked, using a new unique fixture, then perform exact-ID verification and cleanup. Fixture 1 remains untouched.
