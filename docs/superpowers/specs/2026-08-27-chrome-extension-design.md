...
Status: Core implementation and local acceptance verified; post-change production checkpoint remains
...
**Default AI behavior:** AI autofill is on by default for accounts with a provider API key and model configured in Boopmark Settings. Opening the popup automatically invokes the server's suggestion pipeline; no extension opt-in, Autofill click, or second provider-key entry is required. Respect an explicit account-level AI-off setting. Read-only production verification on 2026-08-28 confirmed AI enabled, provider key configured, and model configured; this verifies eligibility, not a successful live AI call.
...
Implementation now lives in [`extensions/chrome/`](../../../extensions/chrome/README.md), with a pinned standalone agent-browser harness, durable per-account create idempotency, and deterministic regression tests. The 2026-08-28 production record verified the headed pre-idempotency build, native toolbar capture, source-consistent AI enrichment, reviewed-value save, refreshed production library, and separately authorized cleanup of fixture 3; fixture 1 remains untouched. The current local build then proved empty/partial autofill behavior, delayed-result edit/clear protection, native outside-dismiss/reopen, zero pre-invocation requests, revoked/reconnect/disconnect cleanup, whole-browser unknown-operation recovery without replay, and the ambiguous transport case with two wire attempts carrying one sanitized operation group and exactly one stored bookmark. Acceptance remains open only for deploying the new server contract and running the bounded post-change production checkpoint, including confirming the deployed AI provenance invariant. A claim that remains pending forever after a server crash is a documented resilience follow-up; the v1 client does not automatically replay an unknown operation and an explicit user retry receives a new operation key. No deployed-source hash or direct provider logs were verified, so the nonempty production suggestion tags remain a source-backed inference rather than definitive provider proof. See the [QA runbook and evidence record](../../chrome-extension-qa.md) for current results and the remaining gate. Connection-screen privacy copy explains automatic metadata/AI processing. Store publishing, OAuth-style connection, and existing-tag-aware suggestions remain follow-ups.
